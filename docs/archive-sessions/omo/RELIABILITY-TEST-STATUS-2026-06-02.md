...
- **6/6 functional sandbox tests PASS** on the current build (test_element rebuilt with all this session's changes, 0 regressions): `SandboxIsolationTests`, `SandboxRealProcessTests`, `SandboxAtomicRefDataIntegrityTests`, `SandboxSemaphoreCrossProcessTests`, `SandboxProtocolTests`, `SandboxWaitForResponseTests`. The host survives worker loss + the real cross-process round-trip works — the ship-gate core.
- The 1000-cycle soaks (`SandboxStressTests` #59 = D-9 SIGKILL stress, `SandboxOrderedShutdownTests` #57) "timed out" NOT from a hang/failure but a **test-config bug**: `ELEMENT_RT_STRESS_TEST=ON` (set in build-merged) expands them 5→1000 cycles but their ctest `TIMEOUT` properties (180s/60s) were never bumped → un-passable by construction. Their output shows the host **surviving + recovering each `kill -9`** (`[sandbox] restart: ...` per cycle). **FIXED in `e2bf5754`** (→1800s when RT_STRESS=ON); full D-9 soak re-running for the definitive 1000/1000 green.
