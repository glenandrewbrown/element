...
namespace detail {
inline Node static findGraph (const Node& node)
{
    if (node.isGraph())
        return node;
...
using PropertyArray = juce::Array<juce::PropertyComponent*>;
...
public:
    ...
    Node node() const noexcept { return graph; }
