> 🗄️ **ARCHIVED — SUPERSEDED by the cherry-pick bake-off. Do NOT execute.** The Stitch-first method here was folded into the bake-off, and its per-component DIRECTION PICKS were overturned by the 37 locked verdicts (e.g. Nav = breadcrumb-only, NOT rail-tree; Inspector = docked-tabbed, NOT contextual-floating). The Stitch project named below ("Element - Design Mode" 6302…) is also stale (current = "Element V3 - Component Studio" 5588…). Live successor: `.omo/plans/mvp-bakeoff-plan.md` (method) · `.omo/bakeoff/VERDICTS.md` (picks) · `docs/adr/ADR-011-ui-cherrypick-bakeoff-method.md`.
